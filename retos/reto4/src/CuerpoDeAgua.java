public class CuerpoDeAgua extends ObjetoGeografico {
    ...
    public static String nivel(float irca_recibido) {
        ...
        if (irca_recibido > 80 && irca_recibido <= 100)
            texto_nivel = "INVIABLE SANITARIAMENTE";
        else if (irca_recibido > 35 && irca_recibido <= 80)
            texto_nivel = "ALTO";
        else if (irca_recibido > 14 && irca_recibido <= 35)
            texto_nivel = "MEDIO";
        else if (irca_recibido > 5 && irca_recibido <= 14)
            texto_nivel = "BAJO";
